...
class WorkflowTransformer extends BaseTransformer<BackendWorkflow, TWorkflow> {
  toFrontend(backend: BackendWorkflow): TWorkflow {
    return {
      ...
      last_run: backend.last_run,
      next_run: backend.next_run,
      runs_count: backend.runs_count,
      tags: backend.tags || [],
      created_at: backend.created_at,
      updated_at: backend.updated_at,
      createdBy: backend.created_by ?? undefined,
      createdByUsername: backend.created_by_username ?? undefined,
  ...
  toBackend(frontend: Partial<TWorkflow>): Partial<BackendWorkflow> {
    const out: Partial<BackendWorkflow> = {
      ...
      runs_count: frontend.runs_count,
      tags: frontend.tags,
    };
